...
export function platformStyle(platform?: string): string {
    ...
    return 'bg-cyan-600 text-white border border-white/20';
}
...
export default function ProductCard({ product, onReserve, onBackorder, onQuickView }: Props) {
    ...
    return (
        <div className="group relative bg-[#12121a] border border-[#242430] rounded-xl p-2.5 flex flex-col hover:border-[#22d3ee]/70 hover:shadow-lg hover:shadow-[#22d3ee]/20 transition-all duration-300">
            {/* Carátula */}
            <button
                onClick={() => onQuickView(product)}
                className="relative w-full aspect-[3/4] mb-2 rounded-lg overflow-hidden bg-[#08080c] flex items-center justify-center border border-[#242430] text-left"
            ...
            >
                ...
                {/* Estado (sup. derecha) */}
                <span className={`absolute top-1.5 right-1.5 z-10 text-[9px] font-black px-1.5 py-0.5 rounded shadow ${isSecond ? 'bg-fuchsia-600 text-white' : 'bg-[#2dd4bf] text-zinc-950'}`}>
                    {conditionText(product)}
                </span>
                {/* Joya Épica */}
                {isEpic(product) && (
                    <span className="absolute bottom-1.5 right-1.5 z-10 text-[9px] font-black px-1.5 py-0.5 rounded bg-gradient-to-r from-[#22d3ee] to-[#2dd4bf] text-white shadow" style={{ textShadow: '0 0 6px rgba(139,92,246,0.9)' }}>
                        💎 ÉPICO
                    </span>
                )}
                {/* Descuento */}
                ...
                {product.stock > 0 ? (
                    <>
                        ...
                        <button
                            ...
                            className="w-full py-1.5 rounded-lg text-[11px] font-bold text-[#cbd5e1] bg-[#242430]/50 hover:bg-[#242430] border border-[#22d3ee]/30 hover:border-[#22d3ee]/60 transition"
                        >
                    ...
                    <>
                        ...
                        <button
                            ...
                            className="w-full py-1.5 rounded-lg text-[11px] font-bold text-[#cbd5e1] bg-[#242430]/50 hover:bg-[#242430] transition"
                        >
                            Encargar producto
                        </button>
                    </>
