...
interface Props {
    ...
    // Enlace de WhatsApp estructurado (mensaje "NUEVO PEDIDO"). Si se omite,
    // se usa el mensaje genérico de comprobante.
    comprobanteHref?: string;
}

// Botón reutilizable para copiar un valor al portapapeles con feedback visual.
function CopyButton({ value, label }: { value: string; label?: string }) {
    ...
    return (
        <button
            type="button"
            onClick={handleCopy}
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold transition ${copied ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-800 hover:bg-slate-700 text-slate-200'}`}
        >
...
export default function PaymentInfo({ orderCode, amount, isFullPayment = false, comprobanteHref }: Props) {
    ...
    return (
        <div className="text-left">
            ...
            {/* Yape / Plin */}
            <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 mb-2">
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5">
                        <span className="inline-block px-2 py-0.5 rounded bg-[#742284] text-white text-[10px] font-black">YAPE</span>
                        <span className="inline-block px-2 py-0.5 rounded bg-[#00bcd4] text-white text-[10px] font-black">PLIN</span>
                    </div>
                    <CopyButton value={PAYMENT_INFO.yapePlinNumber} label="Copiar número" />
                </div>
                <p className="text-lg font-black text-white tracking-wider text-center">{PAYMENT_INFO.yapePlinNumber}</p>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-2 text-xs mb-4">
                ...
                {PAYMENT_INFO.bcpCci && (
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-slate-300 font-mono flex-1 text-right truncate">{PAYMENT_INFO.bcpCci}</span>
                ...
                {PAYMENT_INFO.interbank && PAYMENT_INFO.interbank !== '000-0000000000' && (
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <CopyButton value={PAYMENT_INFO.interbank} label="Copiar cuenta" />
                    </div>
                )}
            </div>
