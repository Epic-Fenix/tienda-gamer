...
const PLATFORMS = ['PS5', 'PS4', 'PS3', 'Nintendo Switch', 'Xbox', 'Otra / No sé'];

// Arma el mensaje de "búsqueda de juego" para WhatsApp.
function buildSearchWhatsappLink(params: { name: string; phone: string; platform: string; title: string; budget: string }) {
    const { name, phone, platform, title, budget } = params;
    const msg =
        `🔎 *BUSCO UN JUEGO - SCOTT GAMES*\n` +
        `----------------------------------\n` +
        `Cliente: ${name || '—'} | Celular: ${phone || '—'}\n` +
        `Juego que busco: ${title}\n` +
        `Consola: ${platform}\n` +
        `Dispuesto a pagar: ${budget.trim() !== '' ? `S/. ${budget}` : '(por definir)'}\n` +
        `----------------------------------\n` +
        `¿Me lo pueden conseguir? ¡Gracias!`;
    return `https://wa.me/${PAYMENT_INFO.whatsappDigits}?text=${encodeURIComponent(msg)}`;
}

export default function GameRequestModal({ onClose }: Props) {
    ...
    const waLink = buildSearchWhatsappLink({ name, phone, platform, title, budget });
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-start justify-between mb-1">
                    <h2 className="text-lg font-bold text-white">🔎 ¿Buscas un juego? Te lo conseguimos</h2>
                    <button onClick={onClose} className="text-slate-400 hover:text-white text-2xl leading-none" aria-label="Cerrar">×</button>
                </div>
                <p className="text-xs text-slate-400 mb-4">Dinos qué juego buscas, para qué consola y cuánto estás dispuesto a pagar. Te respondemos por WhatsApp.</p>
                ...
                <div className="space-y-3 text-xs">
                    ...
                    <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 space-y-3">
                        <p className="text-[11px] font-bold uppercase tracking-wider text-indigo-300">El juego que buscas</p>
                        <div>
                            <label className="text-slate-400 block mb-1">Nombre del juego *</label>
                            <input required type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white focus:outline-none focus:border-indigo-500" placeholder="Ej. Bloodborne, Silent Hill 2..." />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="text-slate-400 block mb-1">Consola</label>
                            ...
                            <div>
                                <label className="text-slate-400 block mb-1">Dispuesto a pagar (S/.)</label>
                                <input type="number" min="0" value={budget} onChange={(e) => setBudget(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white focus:outline-none focus:border-indigo-500" placeholder="Ej. 120" />
                            </div>
                        </div>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-500 text-center">* Completa nombre, celular y el nombre del juego para habilitar el envío.</p>
                </div>
            </div>
        </div>
    );
