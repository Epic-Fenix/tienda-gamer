...
const DEFAULT_SLIDES: Slide[] = [
  { title: '¡Nuevos ingresos PS5!', subtitle: 'Marvel’s Spider-Man 2 y más lanzamientos ya en stock físico.', gradient: 'from-[#242430] via-[#0e7490] to-[#2563eb]', image_url: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2651280/library_hero.jpg', targetSlug: 'spider-man-2-ps5' },
  { title: 'Preventas exclusivas', subtitle: 'Asegura tu juego con solo 20% y recógelo el día de estreno.', gradient: 'from-[#7c1d6f] via-[#22d3ee] to-[#134e4a]', image_url: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1245620/library_hero.jpg', targetSlug: 'elden-ring-sote-ps5' },
...
const PROMO_SLIDES: Slide[] = [
  ...
  },
  {
    ...
    gradient: 'from-[#134e4a] via-[#0891b2] to-[#2dd4bf]',
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#08080c] bg-[radial-gradient(120%_80%_at_50%_-10%,#15151f_0%,#08080c_55%)] text-slate-100">
      {/* TopBar */}
      <div className="bg-[#0d0520] border-b border-[#242430]/60 text-[11px]">
      ...
      {/* Navbar */}
      <header className="sticky top-0 z-30 bg-[#08080c]/95 backdrop-blur border-b border-[#242430]/60">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-xl mx-auto">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8891a8]">
            ...
            <input
              ...
              className="w-full bg-[#12121a] border border-[#242430] rounded-xl pl-9 pr-16 py-2.5 text-sm text-white placeholder-[#8891a8] focus:outline-none focus:border-[#22d3ee]"
            />
          </div>
          <button onClick={openCart} className="relative shrink-0 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black text-zinc-950 bg-[#fcd34d] hover:bg-[#fbbf24] glow-amber transition">
            ...
            {count > 0 && <span className="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 px-1 rounded-full bg-[#22d3ee] text-white text-[11px] font-black flex items-center justify-center border-2 border-[#08080c]">{count}</span>}
          </button>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-[#242430] flex shadow-2xl">
            <div className="flex w-full transition-transform duration-700 ease-out" style={{ transform: `translateX(-${slide * 100}%)` }}>
              {slides.map((b, i) => (
                <div
                  key={i}
                  className={`min-w-full min-h-[220px] sm:min-h-[340px] md:min-h-[460px] relative overflow-hidden flex items-center bg-gradient-to-r ${b.gradient || 'from-[#242430] via-[#0e7490] to-[#2563eb]'}`}
                >
        ...
        <div className="relative group flex items-center">
          <button onClick={() => scrollCategories('left')} aria-label="Categorías anteriores" className="hidden md:flex absolute left-0 z-20 h-full items-center justify-center px-2 bg-gradient-to-r from-[#08080c] via-[#08080c]/80 to-transparent text-white opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex gap-2 min-w-max pb-1">
            {CATEGORIES.map((c) => (
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold whitespace-nowrap border transition ${categoryKey === c.key ? 'bg-[#fcd34d] text-zinc-950 border-[#fcd34d]' : 'bg-[#12121a] text-[#cbd5e1] border-[#242430] hover:border-[#22d3ee]'}`}
              >
          ...
          <button onClick={() => scrollCategories('right')} aria-label="Categorías siguientes" className="hidden md:flex absolute right-0 z-20 h-full items-center justify-center px-2 bg-gradient-to-l from-[#08080c] via-[#08080c]/80 to-transparent text-white opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {/* Grilla de géneros */}
        <section>
          <p className="font-gamer text-[11px] font-black uppercase tracking-wider text-[#8891a8] mb-2">Explora por género</p>
          <div className="flex flex-wrap gap-2">
            {GENRES.map((gname) => (
              <button
                key={gname}
                onClick={() => setGenre((g) => (g === gname ? '' : gname))}
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition ${genre === gname ? 'bg-[#22d3ee] text-white border-[#22d3ee]' : 'bg-[#12121a] text-[#cbd5e1] border-[#242430] hover:border-[#22d3ee]'}`}
              >
                {gname}
              </button>
            ))}
            {(genre || categoryKey !== 'todos' || searchTerm || minPrice || maxPrice || onlyInStock || sortBy !== 'recientes') && (
              <button onClick={() => { setGenre(''); setCategoryKey('todos'); setSearchTerm(''); setMinPrice(''); setMaxPrice(''); setOnlyInStock(false); setSortBy('recientes'); }} className="px-3 py-1.5 rounded-full text-xs font-bold text-[#8891a8] hover:text-white underline">
                Limpiar
              </button>
            )}
          </div>
        ...
        <section className="flex flex-wrap items-center gap-3 bg-[#12121a] border border-[#242430] rounded-xl p-3">
          <span className="text-xs font-bold text-white">{filteredProducts.length} resultados</span>
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-[#8891a8] font-semibold">Precio</span>
            <input type="number" min="0" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} placeholder="Mín" className="w-16 bg-[#08080c] border border-[#242430] rounded-lg px-2 py-1.5 text-xs text-white placeholder-[#4b5563] focus:outline-none focus:border-[#22d3ee]" />
            <span className="text-[#4b5563] text-xs">—</span>
            <input type="number" min="0" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} placeholder="Máx" className="w-16 bg-[#08080c] border border-[#242430] rounded-lg px-2 py-1.5 text-xs text-white placeholder-[#4b5563] focus:outline-none focus:border-[#22d3ee]" />
          </div>
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={onlyInStock} onChange={(e) => setOnlyInStock(e.target.checked)} className="w-4 h-4 accent-[#22d3ee]" />
            <span className="text-[11px] text-[#cbd5e1] font-semibold">Solo en stock</span>
          </label>
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-[11px] text-[#8891a8] font-semibold">Ordenar</span>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="bg-[#08080c] border border-[#242430] rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-[#22d3ee]">
        ...
        <section id="catalogo" className="scroll-mt-20">
          {loading ? (
            <p className="text-center text-[#8891a8] py-10">Cargando catálogo...</p>
          ) : filteredProducts.length === 0 ? (
            <p className="text-center text-[#8891a8] py-10">No se encontraron productos con esos criterios.</p>
          ) : (
            <>
              ...
              {visibleCount < filteredProducts.length && (
                <div className="flex flex-col items-center gap-2 mt-8">
                  ...
                  <span className="text-[11px] text-[#8891a8]">Mostrando {visibleCount} de {filteredProducts.length}</span>
                </div>
              )}
            </>
          )}
        ...
        {/* Footer */}
        <footer className="mt-10 pt-8 border-t border-[#242430]/60">
          <div className="rounded-2xl border border-[#242430] bg-gradient-to-r from-[#12121a] to-[#242430] p-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
            <a href={CONTACT.whatsappSalesLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-xl border border-[#242430] bg-[#12121a] px-4 py-3 hover:border-[#25d366]/60 transition">
              <span className="text-lg">🟢</span>
              <span className="min-w-0">
                <span className="block text-[10px] uppercase tracking-wider text-[#8891a8] font-bold">WhatsApp Ventas</span>
                <span className="block text-sm font-bold text-white truncate">{CONTACT.whatsappSales}</span>
              </span>
            </a>
            <a href={`https://wa.me/${CONTACT.whatsappSupportDigits}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-xl border border-[#242430] bg-[#12121a] px-4 py-3 hover:border-[#25d366]/60 transition">
              <span className="text-lg">💬</span>
              <span className="min-w-0">
                <span className="block text-[10px] uppercase tracking-wider text-[#8891a8] font-bold">WhatsApp Soporte</span>
                <span className="block text-sm font-bold text-white truncate">{CONTACT.whatsappSupport}</span>
              </span>
            </a>
            <a href={`mailto:${CONTACT.email}`} className="flex items-center gap-2 rounded-xl border border-[#242430] bg-[#12121a] px-4 py-3 hover:border-[#22d3ee]/60 transition">
              <span className="text-lg">✉️</span>
              <span className="min-w-0">
                <span className="block text-[10px] uppercase tracking-wider text-[#8891a8] font-bold">Correo</span>
            ...
            <a
              ...
              className="flex items-center gap-2 rounded-xl border border-[#242430] bg-[#12121a] px-4 py-3 hover:border-[#2dd4bf]/60 transition"
            >
              <span className="text-lg">📍</span>
              <span className="min-w-0">
                <span className="block text-[10px] uppercase tracking-wider text-[#8891a8] font-bold">Ubicación</span>
