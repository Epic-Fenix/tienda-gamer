...
// Enlace a la boleta/orden de un pedido (con token secreto si se provee).
export const orderUrl = (code: string, token?: string | null) =>
    `${SITE_URL}/order/${code}${token ? `?t=${token}` : ''}`;
